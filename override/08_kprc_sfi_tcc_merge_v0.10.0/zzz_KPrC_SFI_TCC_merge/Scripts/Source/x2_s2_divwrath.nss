...
/*
	...
	a +3 bonus on attack rolls, damage, and saving throws for a
	number of rounds equal to her Charisma modifier (minimum 1
	round).
	
	
	Reeron modified on 3-13-07 
	Removed Epic portion as it no longer exists per the 3.5 rules description above. 
	This is a true damage reduction effect and therefore Adamantine should not bypass it. 
	Changed the DamageReduction effect to reflect this. 
	Was calling wrong spell id in signal event. Changed to getspellid().
...
void main()
{
	...
	int nDmgRedP = 0; //Setting this to other than 0 makes the Damage Reduction not function properly.
	//int nDmgType = DR_TYPE_GMATERIAL;
	int nDmgType = DR_TYPE_NONE; //With this set to DR_TYPE_NONE, nDmgRedP must be set to zero
	//                             as per the EffectDamageReduction script description.   
