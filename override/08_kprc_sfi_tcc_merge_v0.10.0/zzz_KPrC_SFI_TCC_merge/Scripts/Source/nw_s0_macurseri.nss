...
/*
    ...
    Corrected heal amount to 3d8 (not 4d8) +1 per caster level
    Shouldn't automatically hurt friendly undead anymore.
	...
	//Check if racial type is undead
     if (GetRacialType(oTarget) == RACIAL_TYPE_UNDEAD )
     {
     	if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
     	{
             ...
             if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
             {
                 ...
                 nModify = ApplyMetamagicVariableMods(nModify, 24 + nBonus);
     ...
     else
     {
     	...
     	if (spellsIsTarget(oTarget, SPELL_TARGET_ALLALLIES, OBJECT_SELF))
     	{
             ...
             nHP = ApplyMetamagicVariableMods(nHP, 24);
