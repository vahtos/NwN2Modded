...
This version should be used with NWN2 1.23 or later with or without the MotB or SoZ 
...
The AI scans some key game files (2das) the first time a new version of AI is installed 
and run. If you add or update some other content that changes spells.2da, run the 
following command to reload the AI cache:
`   					<this can vary by language>
DebugMode 1
rs hench_o0_initcache			<should indicate script has successfully run>
DebugMode 0
`					<this can vary by language>
...
Fleeing (creature examine menu only)
Familiars and animal companions can be told to flee if they are hurt or the fight is too 
challenging.
...
Monster Healing and resting.
Monsters will heal and rest outside of combat.
...
Associate Inventory
For animal companions and familiars. Creates a bag in your inventory that are equipped 
if possible, on the associate. An animal companion will equip light armor, otherwise no 
weapons or armor will be equipped. No items that require hands like wands or scrolls can 
be used either.
...
about issues from the very first release. Use of NWN2Packer provided by tani. Use of  
Advanced Script Compiler for NWN2 provided by Skywing.

History

V2.5, Dec 13, 2020

Improve finding subspells when searching for spell talents. Add workaround for game 
spell resistance bug for items. Improve warlock spellcasting to better account for spell 
level in saves. Fix and improve knockdown and feint combat usage. Add support for 
flourish and impromptu sneak attack. Account for greater restoration full heal in AI. Fix 
issue with persistent aura duplicate cast when moving to target. Support for dragon 
spellcasting of clerical spells as arcane sorcerer. Fixes for spontaneous metamagic 
spellcasting being used with other spellcasting levels.

V2.4, Nov 5, 2020

Metamagic support for sorcerers, bards, favored souls and spirit shamans. Spells with sub 
spells (i.e. protection from alignment) will get cast but with no metamagic due to game 
limitation. All other metamagic should get used when appropriate except for quicken. 
Persistent spell is only used for long term buffing outside of combat for haste and divine 
favor. Rework dispel usage to debuff enemies and reduce chance of TMI errors. Rework 
handling of area of effect spells. Now more likely to get out of area even if in melee 
combat and will attempt to move to a better location instead of just directly moving away 
from area of effect spell.

V2.3, Oct 19, 2020

Modifications to at will spells. Additional script calls to prevent inter party fights 
especially those due to AoE spells. Animal companion and familiar flee options when 
hurt or facing a challenging enemy. Animal companion and familiar inventory access. 
Updates for Kaedrin's PrC pack 1.41.4. Add support for casting free or swift actions in 
AI. Golem gas breath is now set to use this. Improved support for stealthed, invisible, and 
ethereal opponents and allies. Reworked monster heartbeat animations. Added support 
for avian, herbivore, and omnivore behavior. Monsters will return to starting location 
after combat. Monster wander will return to starting location if you move away. Added 
global option for monster heal and rest outside of combat.
