...
*/
/*
   Modified by Reeron on 9-14-07
   Now only does one save, instead of incorrectly 
   doing two saves. Once the save is done, the script 
   then splits up the damage into half by fire and 
   half by divine if any damage is applicable. Save type 
   is versus spells, not divine or fire. Immunity to either 
   shouldn't matter for the save. When the damage is applied, 
   immunity will correctly block the appropriate damage.
  ...
  while ( GetIsObjectValid(oTarget) )
  {
       if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
	   {
    	   ...
    	   { 
                ...
                //{
                    eHoly = EffectDamage( nDamage/2, DAMAGE_TYPE_DIVINE );
                //}
