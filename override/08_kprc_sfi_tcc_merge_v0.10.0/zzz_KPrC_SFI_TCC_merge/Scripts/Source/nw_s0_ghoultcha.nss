...
//  Reeron modified on 4-1-07
//  Attack penalty was never linked to the other effects.
//  Saving Throw decrease was linked twice, not once.
//  GetAreaofEffectCreator() must be passed into saving throw,
//  otherwise invalid saves could occur.
//  Immunity to poison prevents the effects as per PnP.
//
...
void main()
{
    ...
    {
      if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget) && !MySavingThrow(SAVING_THROW_FORT, oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_POISON, oCreator))
        {
