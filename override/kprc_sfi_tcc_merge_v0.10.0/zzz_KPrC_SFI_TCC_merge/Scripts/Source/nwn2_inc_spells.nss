...
//
//
//        Reeron modifed on 3-14-07
//        Modified Devour Magic Dispelling callback so temporary hitpoints aren't stacked
//
//        Reeron modified on 9-30-07
//        now compatible with patch 1.10
//
//        Reeron modified on 3-31-08
//        Meteor Swarm only has 4 meteors as per PnP.
//
//        Reeron modified on 4-12-08
//        Added Enlarge Person (Duergar Racial Ability) to HasSizeIncreasingSpellEffect() check.
//        Devour Magic callback won't try to remove it's own temporary hitpoints anymore.
//
//        Reeron modified on 5-4-09
//        Updated for patch 1.22.5588
//
//        Reeron modified on 8-20-11
//        Fatigue and Exhaustion can now be removed by Heal, Mass Heal, Lesser Restoration, 
...
//RedRover updated 4/30/2012
//  added the following changes from Kaedrin's scripts:
//  Devour Magic callback - changed eHitPoints to SupernatrualEffect
//  Pilfer Magic callback - changed duration to 10 rounds + Arcane Trickster level
//  Apply Bard songs to party - changed NPC faction check
//  Mass Heal (+Warpriest) - capped at 250, up from 150. Heal still 150
//  Harm (+Mass) - Removed delay to prevent some issues with delayed saving throws
...
int HasSizeIncreasingSpellEffect( object oTarget )
{
	...
	if ( GetHasSpellEffect( 803, oTarget ) ) //Enlarge Person (Duergar Racial Ability)
		return TRUE;
...
//	eHitPoints = ExtraordinaryEffect(eHitPoints); //RedRover - changing to following line per Kaedrin's update
	eHitPoints = SupernaturalEffect(eHitPoints); //RedRover; per Kaedrin - FIX: prevents Devour Magic from canceling its own effects
...
//	float  fDuration 	= RoundsToSeconds(10); // 10 rounds //RedRover - changed to following line, per Kaedrin's updates
	float  fDuration 	= RoundsToSeconds(10) + GetLevelByClass(CLASS_TYPE_ARCANETRICKSTER, oCaster); //RedRover: per Kaedrin's update
...
{
//    object oLeader = GetFactionLeader( oCaster );//RedRover: per Kaedrin's changes, changed to following line
    ...
    while ( GetIsObjectValid( oTarget ) )
    {
		...
		//if ( GetIsObjectValidSongTarget(oTarget) )//RedRover: changed to following line per Kaedrin's update
...
effect EffectFatigue()
{
	...
	eRet = SetEffectSpellId(eRet, -998);// Now I can remove it with Heal, Mass Heal, Lesser Restoration, Restoration, and Greater Restoration.
...
effect EffectExhausted()
{
	...
	effect eRet = EffectLinkEffects (eStrPenalty, eDexPenalty);
	eRet = EffectLinkEffects(eRet, eMovePenalty);
	eRet = SetEffectSpellId(eRet, -999);// Now I can remove it with Heal, Mass Heal, Restoration, and Greater Restoration.
	eRet = ExtraordinaryEffect(eRet);
	return (eRet);
}
...
//              capped at 150.
//-----------------------------------------------------------------------------
void HealHarmTarget( object oTarget, int nCasterLevel, int nSpellID, int bIsHealingSpell )
{
	int nHealAmt = ClampInt( 10*nCasterLevel, 0, 150 );
	
	if (nSpellID == 114 || nSpellID == 965)         //RedRover: added per Kaedrin's update
		nHealAmt = ClampInt(10*nCasterLevel,0,250);   //RedRover: added per Kaedrin's update
	
	int bHarmTouchAttack = FALSE; //No touch attack is needed to zap people //RedRover: removed per Kaedrin's update
	//if (!bIsHealingSpell)
...
int HealHarmObject( object oTarget, effect eVis, effect eVis2, int nCasterLvl, int nSpellId )
{
	...
	{
	 	bRet = TRUE;
		//RedRover: Per Kaedrin - FIX: Removed delay to prevent some issues with delayed saving throw in DoHarming()
