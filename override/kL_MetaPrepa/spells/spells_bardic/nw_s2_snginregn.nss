...
void RunPersistentSong(object oCaster, int iSpellId)
{
	if (GetCanBardSing(oCaster))
	{
		...
		if (FindEffectSpellId(EFFECT_TYPE_BARDSONG_SINGING) == iSpellId)
		{
			...
			if (iLevel > 11)
			{
				iRegen += (iLevel - 7) / 5; // +1 every 5 levels starting at level 7
			}

			if (GetHasFeat(FEAT_EPIC_INSPIRATION, oCaster))		iRegen += 2;
			if (GetHasFeat(FEAT_SONG_OF_THE_HEART, oCaster))	iRegen += 1;

			effect eRegen	= EffectRegenerate(iRegen, RoundsToSeconds(2));
			effect eDur		= EffectVisualEffect(VFX_HIT_BARD_INS_REGENERATION);
			eDur			= EffectLinkEffects(eRegen, eDur);
...
void main()
{
//	if (kL_iDebug) SendMessageToPC(GetFirstPC(FALSE), "Run ( nw_s2_snginregn ) - " + GetName(OBJECT_SELF));
