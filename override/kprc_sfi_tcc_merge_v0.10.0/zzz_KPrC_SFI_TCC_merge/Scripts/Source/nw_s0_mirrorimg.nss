...
/*
    ...
    Reeron modified on 3-24-07
    Used wrong cacluation for the AC of the created images. 
    Changed to use PnP description for caculating the AC. 
    Corrected to use AC=(10+dexterity modifier) like it's supposed to. 
    Also, removed the level 15 cap as this is not per PnP.
    Maximized corrected to 8 images, not 4. Dispel Magic now
    removes all effects properly.
...
// End of Spell Cast Hook
	...
	int nACBonus = 10 + GetAbilityModifier(ABILITY_DEXTERITY, oTarget);
	
	//Determine how many images will be created
    int nImages = d4( 1 ) + ( nCasterLvl / 3 );
	
    //Enter Metamagic conditions
    nImages = ApplyMetamagicVariableMods(nImages, 8);
    //nImages += nCasterLvl;
...
//        effect eVis = EffectVisualEffect(VFX_IMP_HOLY_AID); //Don't need this.
        ...
        effect eDispel = EffectOnDispel(0.0f, RemoveEffectsFromSpell(oTarget, SPELL_MIRROR_IMAGE));
